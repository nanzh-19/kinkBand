...
c=s^-1;            %求解刚度矩阵
CSE=[c(1,1) c(1,2) c(1,3) c(1,3);c(2,1) c(2,2) c(2,3) c(2,3);c(3,1) c(3,2) c(3,3) c(3,3);c(3,1) c(3,2) c(3,3) c(3,3)];
f11=0.98519;
f22=sqrt(-c(2,2)*(c(1,2)*f11^2-c(1,2)-c(2,2)))/c(2,2);
f12=0;
f21=0;
F=[f11 f12;f21 f22];
S=[-2.897 0;0 -2.861e-08];   %S为应力
...
duf(4,2)=duf(2,4);
duf(4,3)=duf(3,4);
C=zeros(4,4);  %CSE推出的偏P/偏F
C(1,1)=S(1,1)+F(1,:)*CSE([1,4],[1,4])*F(1,:)';
C(2,2)=S(2,2)+F(2,:)*CSE([3,2],[3,2])*F(2,:)';
C(3,3)=S(2,2)+F(1,:)*CSE([3,2],[3,2])*F(1,:)';
C(4,4)=S(1,1)+F(2,:)*CSE([1,4],[1,4])*F(2,:)';
C(1,2)=F(1,:)*[CSE(1,3) CSE(1,2);CSE(3,4) CSE(2,4)]*F(2,:)';
C(1,3)=S(1,2)+F(1,:)*[CSE(1,3) CSE(1,2);CSE(3,4) CSE(2,4)]*F(1,:)';
C(2,3)=F(2,:)*CSE([3,2],[3,2])*F(1,:)';
C(1,4)=F(1,:)*CSE([1,4],[1,4])*F(2,:)';
C(2,4)=S(1,2)+F(2,:)*[CSE(1,3) CSE(3,4);CSE(1,2) CSE(2,4)]*F(2,:)';
C(3,4)=F(1,:)*[CSE(1,3) CSE(3,4);CSE(1,2) CSE(2,4)]*F(2,:)';
temp=diag(C);
C=C+C'-diag(temp)
